...
#ifdef SDL
				if (is_ym2608) {
					// from PC-88 machine language master bible
					if (passed_usec(clock_busy) < 4.25) {
						status |= 0x80;
					}
					else {
						busy = false;
...
#ifdef SDL
	if ((reg27 & 0xc0) == 0x80) {
		// csm mode (keyon/keyoff from timer-a)
		mix_sound_block();
	}
#endif // SDL
#endif
...
#endif
#ifdef SDL
	reg27 = state_fio->FgetUint8();
#endif // SDL
	return true;
}

#ifdef SDL
void YM2203::SetRate(uint c, uint r, bool ip)
{
#ifdef HAS_YM2608
	if(is_ym2608) {
		opna->SetRate(c, r, ip);
